...
class automat
{
	private:
		...
		int inputs; //wejscia
		int **stateArr; //tablica stan�w
		char **output; //tablica wyjs�
	...
	public:
		...
		void AllocAutomat(int ty, int st, int in)
		{
			...
			//alokacja pami�ci na stany automatu dla poszczeg�lnych wejs�
			stateArr = new int*[st];
			for(int i = 0; i < st; ++i)
				stateArr[i] = new int[in];

			//alokacja pami�ci na tablice wyjscia
			if(type == 0) //mealy
			{
				output = new char*[st];
				for(int i = 0; i < st; ++i)
		...
		void ReadAutomatFromFile(FILE *f)
		{
			...
			for(int i = 0; i < states; ++i) //wy�wietlanie tabeli z danymi
			{
				for(int j = 0; j < inputs; ++j)
				{
		...
		void CreateUngerPaullTable()
		{
			...
			for(int i = 0; i < states; ++i) //wype�nianie
			{
				...
				{
					bool match = false;
					//sprawdzenie dopasowania wyj�cia
					...
					if(match)
					{
						...
						for(int k = 0; k < inputs; ++k)
						{
							...
							{
								if(stateArr[i][k] > stateArr[j][k]) //segregacja wynik�w dla u�atwienia
								{
			...
			//pozbycie si� z tablicy sprzeczno�ci
			...
			for(int times = 0; times < iteration; ++times)
			{
				for(int i = 0; i < states; ++i)
				{
					...
					{
						if(ungerPaull[j][i].size() > 0 && ungerPaull[j][i][0] == -2) //gdy znajdzie si� sprzeczno�� trzeba poszuka� p�l kt�re
						{
							for(int m = 0; m < states; ++m) //przechodzenie przez tablice paulliego
							{
								for(int n = 0; n < states - 1; ++n)
								{
									for(int k = 0; k < ungerPaull[n][m].size(); ++k)
									{
										...
										if(ungerPaull[n][m][0] == -2) break; //je�li pole ju� jest zaznaczone jako sprzeczno�� to pomija
				...
				} //koniec pozbywania si� niesprzeczno�ci
			}
			...
			for(int i = 0; i < states; ++i)
			{
				for(int j = 0; j < states - 1; ++j)
				{
					for(int k = 0; k < ungerPaull[j][i].size(); k+=2)
					{
						...
						if(ungerPaull[j][i][0] == -2) break; //je�li pole ju� jest zaznaczone jako sprzeczno�� to pomija
			...
			for(int i = 1; i < states; ++i) //wy�wietlanie tablicy paulliego
			{
				for(int j = 0; j < states - 1; ++j)
				{
					for(int k = 0; k < ungerPaull[j][i].size(); ++k)
					{
						//-3 nic nie oznacza | -4 to V czyli wszystki zgodne | -2 to X czyli sprzeczno��
		...
		int OrganizeFamily() //��czy stany w jaknajwi�ksze grupy oraz zlicza ilo�� po��czonych grup by je zwr�ci�
		{
		...
		void FindMaxFamily()
		{
			...
			for(int i = states - 2; i >= 0; --i) //wy�wietlanie tablicy paulliego
			{
